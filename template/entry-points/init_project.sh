...
# ==========================================================================
# RÉFÉRENCES CROISÉES:
# - Ce fichier est référencé dans: [template/entry-points/setup_project.sh:203]
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" || "$OSTYPE" == "cygwin" ]]; then
    ...
    if [ -f "${PARENT_DIR}/utils/setup.bat" ]; then
        echo "Exécution de ${PARENT_DIR}/utils/setup.bat..."
        cmd.exe /c "${PARENT_DIR}\\utils\\setup.bat"
    ...
    if [ -f "${PARENT_DIR}/utils/setup.sh" ]; then
        echo "Exécution de ${PARENT_DIR}/utils/setup.sh..."
        chmod +x "${PARENT_DIR}/utils/setup.sh"
        "${PARENT_DIR}/utils/setup.sh"
    ...
    if [ -f "${PARENT_DIR}/utils/setup.sh" ]; then
        echo "Tentative d'exécution de ${PARENT_DIR}/utils/setup.sh..."
        chmod +x "${PARENT_DIR}/utils/setup.sh"
        "${PARENT_DIR}/utils/setup.sh"
...
if [ -f "${PARENT_DIR}/utils/configure_vscode_for_flutter.sh" ]; then
    echo "Configuration de VS Code pour Flutter..."
    chmod +x "${PARENT_DIR}/utils/configure_vscode_for_flutter.sh"
    "${PARENT_DIR}/utils/configure_vscode_for_flutter.sh"
...
print_header "Installation des dépendances"
if [ -f "${PARENT_DIR}/utils/install_dependencies.sh" ]; then
    echo "Installation des dépendances du projet..."
    chmod +x "${PARENT_DIR}/utils/install_dependencies.sh"
    "${PARENT_DIR}/utils/install_dependencies.sh"
...
echo ""
if [[ $REPLY =~ ^[oO]$ ]]; then
    if [ -f "${PARENT_DIR}/utils/cleanup_init_files.sh" ]; then
        echo "Exécution du script de nettoyage..."
        chmod +x "${PARENT_DIR}/utils/cleanup_init_files.sh"
        "${PARENT_DIR}/utils/cleanup_init_files.sh"
    else
        print_warning "Script de nettoyage non trouvé"
    fi
else
    echo "Les fichiers d'initialisation n'ont pas été supprimés."
    echo "Vous pouvez les nettoyer ultérieurement en exécutant ${PARENT_DIR}/utils/cleanup_init_files.sh"
fi
