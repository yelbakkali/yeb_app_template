...
:: ==========================================================================
:: RÉFÉRENCES CROISÉES:
:: - Ce fichier est référencé dans: [template/entry-points/setup_project.bat:59]
...
set "PREREQ_SCRIPT=%PARENT_DIR%utils\check_prerequisites.bat"
...
if exist "%PARENT_DIR%utils\setup.bat" (
    echo Exécution de %PARENT_DIR%utils\setup.bat...
    call "%PARENT_DIR%utils\setup.bat"
...
if exist "%PARENT_DIR%utils\configure_vscode_for_flutter.bat" (
    echo Configuration de VS Code pour Flutter...
    call "%PARENT_DIR%utils\configure_vscode_for_flutter.bat"
...
if /i "%CLEANUP_CHOICE%"=="O" (
    if exist "%PARENT_DIR%utils\cleanup_init_files.bat" (
        echo Exécution du script de nettoyage...
        call "%PARENT_DIR%utils\cleanup_init_files.bat"
    ) else (
        call :print_warning "Script de nettoyage non trouvé"
    )
) else (
    echo Les fichiers d'initialisation n'ont pas été supprimés.
    echo Vous pouvez les nettoyer ultérieurement en exécutant %PARENT_DIR%utils\cleanup_init_files.bat
)
...
echo    Pour fusionner dev vers main : scripts\merge_to_main.bat (à la racine du projet)
echo.
