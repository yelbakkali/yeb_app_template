"""
max climb n1.py
Extraction des tableaux du sous-chapitre Max Climb %N1.

Emplacement : PI/max climb n1/max climb n1.py

Peut etre lance seul :
  python3 "max climb n1.py"
...
def extract_climb_bleed_table(lines, start_idx):
    ...
    while i < len(lines):
        ...
        if parts[0] in ('ENGINE', 'PACKS', 'WING'):
            ...
            # Suite sur ligne suivante si valeurs manquantes
...
def extract_max_climb_n1(pdf_path, page_num):
...
def run(fcom_dir, pi_dir, subchapters):
    ...
    for entry in entries:
        ...
        tables = extract_max_climb_n1(pdf_path, page)
        if tables:
